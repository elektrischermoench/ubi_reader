...
def filename_decrypt(key: bytes, ciphertext: bytes):
    ...
    if len(ciphertext) > AES_BLOCK_SIZE:
        ...
        pad = (AES_BLOCK_SIZE - len(ciphertext) % AES_BLOCK_SIZE) % AES_BLOCK_SIZE
        ...
        if pad > 0:  # Steal ciphertext only if needed (CTS)
            ...
            # Apply padding 
            ciphertext += bytes(plaintext[-pad:])
    ...
    try:
        ...
        for inode in inodes.values():
            ...
            for dent in inode['dent']:
                dent.name = filename_decrypt(dec_key, dent.name)
    except Exception as e:
        error(decrypt_filenames, 'Error', '%s' % e)


def decrypt_symlink_target(ubifs, inodes, dent_node) -> bytes:
    if ubifs.master_key is None:
        return inodes[dent_node.inum]['ino'].data.decode()
    inode = inodes[dent_node.inum]
    ino = inode['ino']
    nonce = lookup_inode_nonce(inodes, inode)
    name_len = ino.data_len - 2
    cit = ino.data[-name_len:-1]
    dec_key = derive_key_from_nonce(ubifs.master_key, nonce)
    lnkname = filename_decrypt(dec_key, cit)
